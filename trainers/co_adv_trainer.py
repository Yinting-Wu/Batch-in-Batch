...
class COAdvTrainer(AdvTrainer):
    ...
    def _do_iter(self, train_data):
        ...
        correct_idx = torch.masked_select(
            ...
            correct)
        wrong_idx = torch.masked_select(
            torch.arange(batch_size).to(self.device), ~correct)
        
        # Use misclassified images as final images.
        X_adv[wrong_idx] = X[wrong_idx].detach()
            # (图片+简单噪声)分类错误的退回原图片
                # 正确的应该是FFGSM分类错误的退回原图片，\
                # 这里弄错了应该
        ...
        torch.cat(
            [
                torch.arange(1, self.c).to(self.device)\
                .view(-1, 1)]*batch_size, 
                dim=1).view(-1, 1, 1, 1) * \
...
class CustomFFGSM(Attack):
    def __init__(self, model, eps=8/255, alpha=10/255):
        super(CustomFFGSM, self).__init__("CustomFFGSM", model, device='cuda:0')
        self.eps = eps
        self.alpha = alpha
